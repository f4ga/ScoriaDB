...
- **Решение:** `Next()` переименован в `Current()` (read-only) — возвращает текущее значение счётчика без мутации. `Current()` и `Increment()` теперь явно разделяют read-only чтение и выделение нового timestamp. `Next()` оставлен как помеченная deprecated совместимая обёртка над `Current()`. См. DEF‑D1, Глава VII.
- **Проверка:** `TestTimestampCurrent`, `TestTimestampIncrement`, `TestTimestampNoDuplicates`, `TestTimestampRace` — проходят с `-race`.

### DEF‑D2 — `maybeCompact` запускает горутину под `Lock`
- **Файлы:** `internal/engine/compaction.go`, `internal/engine/lsm.go`
- **Статус:** ✅ РЕШЕНО (2026‑08‑18)
- **Решение:** `maybeCompact` теперь сигнализирует через буферизованный канал `compactCh` БЕЗ удержания `e.mu.Lock` (только короткий `RLock` для проверки условия). Выделенный worker `compactionWorker` ([`lsm.go`](internal/engine/lsm.go:158)) повторно проверяет условие `len(e.levels[0]) > MaxLevel0Files` под коротким замком перед выполнением `compactLevel0`. Горутина под `Lock` больше не запускается. См. DEF‑D2, Глава XIII.
- **Проверка:** `TestMaybeCompactNonBlocking`, `TestCompactConditionRecheck`, `TestCompactNoDeadlock`, `TestCompactWorkerRecheckCondition` — проходят с `-race`.
...
### LSM‑02 — Flush блокирует запись
- **Файлы:** `internal/engine/flush.go`, `internal/engine/shard.go`, `internal/engine/lsm.go`, `internal/engine/compaction.go`, `internal/engine/sstable/reader.go`
- **Статус:** ✅ РЕШЕНО (2026-08-18)
- **Решение:**
  1. `flushMemTable` теперь собирает кандидатов под коротким `Lock`, затем записывает SSTable **без** блокировки, и только потом обновляет `e.levels` под коротким `Lock`.
  2. `Shard.Get` теперь использует `s.mu.Lock()` для доступа к активной/frozen MemTable, устраняя гонку с `flushMemTable` (которая меняет указатели и вызывает `Close()` → `arena.Reset()` под `s.mu`).
  3. Flushed-ридер публикуется и в `e.levels[0]`, и в `s.levels[0]` шарда; компакшн зеркалит переход уровня на каждый шард — данные остаются видимыми для `Shard.Get`.
  4. Добавлен ref-count (`Acquire`/`Release`) на `sstable.Reader`, чтобы параллельный компакшн не `munmap`'ал mmap-регион во время чтения.
  5. `LastTS` восстанавливается после рестарта из максимума WAL-рекавери по всем шардам (seed `eng.LastTS`).
- **Компромисс:** `Get` платит локальный мьютекс и ref-count на ридеры, но только на короткие операции, не на всё сканирование SSTable. Соответствует трактату (Глава IV.3, Глава XIII, Глава XIV).
- **Тесты:** `TestFlushNonBlocking`, `TestFlushParallelShards`, `TestFlushConsistency`, `BenchmarkFlushWhileWriting`, `TestFlushRace` — PASS с `-race -count=5`.
- **Коммит:** `perf(engine): make flushMemTable non-blocking and fix reader-flush race`

## P1 — Important (v0.4.0–v0.5.0)

| ID | Проблема | Локация | Статус | Достоверность |
|----|----------|---------|:---:|:---:|
| LSM-02 | Flush блокирует запись | `flush.go` | ✅ РЕШЕНО (2026-08-18) | ✔️ |
