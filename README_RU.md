...
## 📊 Производительность (v0.3.0)

*Все бенчмарки запущены на Intel Core i3-1215U (8 потоков), NVMe SSD, Go 1.23+, Linux amd64.*

### MemTable (lock‑free SkipList с аренным аллокатором)

Это **in‑memory слой** — сюда сначала попадают все записи.  
Начиная с v0.3.0, используется lock‑free skip list с zero‑copy ареной.
...
**Улучшения относительно v0.2.2:**
- Get: **+159%** пропускной способности (7.1M → 18.4M), **-49%** задержки (140 → 72 нс)
- Put: **+94%** пропускной способности (1.51M → 2.92M), **-35%** задержки (662 → 432 нс)
- Аллокации: **-80%** (5 → 1 allocs/op)

---

### Движок (LSM + WAL + VLog)

Полный движок хранения — включает WAL, долговечность, MVCC и Value Log.

| Операция | Пропускная способность | Задержка | Аллокации | Примечание |
|----------|------------------------|----------|-----------|------------|
| **Get (существующий ключ)** | ~10M ops/s | ~100 нс | 1 alloc | Быстрый путь (попадание в MemTable) |
| **Get (4KB, VLog)** | ~8.5M ops/s | ~118 нс | 1 alloc | Zero‑copy чтение из mmap |
| **Put (16B)** | 1.12M ops/s | 889 нс | 6 allocs | **В работе** — цель 420 нс |
| **WAL Group Commit** | 17.5M ops/s | 57 нс | 0 allocs | ✅ **в 3 раза быстрее цели** |
| **WAL Sync** | 2.74M ops/s | 365 нс | 0 allocs | ⚠️ цель 170 нс |
| **Scan (100 ключей)** | — | 4.8 мкс | **107 allocs** | ❌ требует переписывания |

**Известные проблемы (исправляются):**
- `PutLarge` → **SIGBUS** (запись за пределы mmap)
- `Scan` → **107 allocs/op** (итератор нужно переписать)
- `VLog` → **переполнение квоты / SIGBUS** (в процессе выяснения)

---

### Сравнение с v0.2.2

| Метрика | v0.2.2 | v0.3.0 | Улучшение |
|---------|--------|--------|-----------|
| **Get (8 ядер)** | 7.1M ops/s | **18.4M ops/s** | **+159%** |
| **Put (8 ядер)** | 1.51M ops/s | **2.92M ops/s** | **+94%** |
| **Аллокации (Get)** | 5 allocs/op | **1 alloc/op** | **-80%** |
...
| **Unified MMap** | 2 syscall/запись | **0 syscall** | **-100%** |
| **SSTable block pooling** | 432 нс | 140 нс | **-67%** |
| **WAL buffer pooling** | 515 нс | 436 нс | **-15%** |

---

Все бенчмарки воспроизводимы командой:
